...
//    Locations.cityLocationsList_1_restartPoint.foreach((city: String) => {
//      val cityNameRegex = ".+?(?=,)".r
...
//      if (next.isDefined) {
//        requestCategoriesToFileChained(accessToken,next.get)
//      }
//    }
  }
//
//  def requestCategoriesToFileChained(accessToken: String, next: String): Unit = {
//    val result = Http(s"https://api.meetup.com/find/topic_categories")
//      .header("Authorization", s"Bearer $accessToken")
//      .option(HttpOptions.readTimeout(60000)).asString
//
